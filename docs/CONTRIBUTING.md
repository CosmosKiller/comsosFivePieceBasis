...
- **OTA (mandatory for every SKU):** `CONFIG_ENABLE_OTA_REQUESTOR=y`, `CONFIG_CHIP_OTA_IMAGE_BUILD=y`, and `CONFIG_DEVICE_SOFTWARE_VERSION_NUMBER` matching `PROJECT_VER_NUMBER` in the app `CMakeLists.txt`. Dual OTA partitions in `partitions.csv`. Call `cosmos_matter_ota_configure()` after `esp_matter::start()`. New apps (including SKU 5) must ship OTA-ready from day one. Details: [BUILD.md](BUILD.md#ota-images). Field rollout: [cosmos-ha-field](https://github.com/CosmosKiller/cosmos-ha-field).
