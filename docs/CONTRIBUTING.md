...
4. Read [HARDWARE.md](HARDWARE.md) for GPIO and carrier notes per SKU.
...
- **Naming / docs:** `snake_case` functions, `*_t` / `*_e` types, Doxygen on public `tasks/*.h` APIs — details in [CODE_STYLE.md](CODE_STYLE.md). Apply Cosmos style to **new and touched** files; keep upstream Matter/CHIP names at API boundaries. A later full rename of older owned code is **Plan B**, bundled with the IDF/esp-matter bump and `endpoint::doorbell` migration — [Post-Beta cleanup combo](POLISH_PLAN.md#post-beta-cleanup-combo--plan-b--toolchain--5a).
...
- **OTA (mandatory for every SKU):** `CONFIG_ENABLE_OTA_REQUESTOR=y`, `CONFIG_CHIP_OTA_IMAGE_BUILD=y`, and `CONFIG_DEVICE_SOFTWARE_VERSION_NUMBER` matching `PROJECT_VER_NUMBER` in the app `CMakeLists.txt`. `PROJECT_VER` must be **`MAJOR.MINOR.PATCH`**. Dual OTA partitions in `partitions.csv`. Call `cosmos_matter_ota_configure()` after `esp_matter::start()`. Bump rules: [RELEASING.md](RELEASING.md). Details: [BUILD.md](BUILD.md#ota-images). Field: [cosmos-ha-field](https://github.com/CosmosKiller/cosmos-ha-field).
