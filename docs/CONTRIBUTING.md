...
- **OTA:** enabled via `CONFIG_ENABLE_OTA_REQUESTOR=y` in shared `sdkconfig.defaults.matter-base`. Each app calls `cosmos_matter_ota_configure()` after `esp_matter::start()`. OTA image build notes: [BUILD.md](BUILD.md#ota-images). HA / field rollout: [cosmos-ha-field](https://github.com/CosmosKiller/cosmos-ha-field).
